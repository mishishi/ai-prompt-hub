    ...
    <div className="flex flex-col lg:flex-row h-full page-enter">
      ...
      <div className="w-full lg:w-[420px] flex-shrink-0 border-b lg:border-b-0 lg:border-r border-[var(--color-bench-border)] bg-[var(--color-bench-elevated)] flex flex-col">
        ...
        <div className="flex-1 flex flex-col px-6 space-y-4 overflow-y-auto pb-4">
          ...
          <div>
            <p className="text-[10px] font-semibold text-[var(--color-bench-muted)] uppercase tracking-wider mb-2.5 flex items-center gap-1"><Lightbulb size={10} />{tq('Quick Start', '快捷开始')}</p>
          ...
          <>
            <div className="px-5 py-3 border-b border-[var(--color-bench-border)] bg-[var(--color-bench-elevated)] flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <span className="text-[11px] font-semibold uppercase tracking-wider text-[var(--color-bench-muted)]">{tq('Output', '生成结果')}</span>
              </div>
            ...
            <div className="flex-1 flex flex-col">
              <div className="px-5 py-2 border-b border-[var(--color-bench-border)] flex items-center justify-between bg-[var(--color-bench-elevated)]">
                <span className="text-xs font-medium text-[var(--color-bench-accent)]">{tq('Editing result...', '编辑中...')}</span>
                <div className="flex items-center gap-2">
                  <button onClick={() => { setResult(editText); setEditing(false); }} className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-medium bg-[var(--color-bench-accent)]/10 text-[var(--color-bench-accent)] hover:bg-[var(--color-bench-accent)]/20 transition-colors">{tq('Apply', '应用')}</button>
                  <button onClick={() => setEditing(false)} className="flex items-center gap-1 px-2 py-1.5 rounded-lg text-xs font-medium text-[var(--color-bench-muted)] hover:text-[var(--color-bench-text)] transition-colors"><X size={12} /></button>
                </div>
              </div>
            ...
            <div className="px-5 py-3 border-t border-[var(--color-bench-border)] bg-[var(--color-bench-elevated)] flex items-center justify-center gap-3">
              <span className="text-xs text-[var(--color-bench-text-dim)]">{tq('Was this helpful?', '对你有帮助吗？')}</span>
              {feedback ? <span className="text-xs text-[var(--color-bench-success)] font-medium">{tq('Thanks!', '感谢反馈！')}</span> : <>
                <button onClick={() => handleFeedback('up')} className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-medium bg-[var(--color-bench-success)]/10 text-[var(--color-bench-success)] hover:bg-[var(--color-bench-success)]/20 transition-colors"><ThumbsUp size={12} />{tq('Helpful', '有帮助')}</button>
                <button onClick={() => handleFeedback('down')} className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-medium bg-[var(--color-bench-error)]/10 text-[var(--color-bench-error)] hover:bg-[var(--color-bench-error)]/20 transition-colors"><ThumbsDown size={12} />{tq('Not helpful', '没帮助')}</button>
              </>}
            </div>
          </>
        )}
